...
## 곁들이는 그림 (trace · source 공통, 모두 선택)

모두 **파일이나 그림을 리포에 두지 않는다.** 카드에는 이름과 숫자만 적고, 화면이 그것으로 주소를 만들거나 도형을 그린다 (D29·D46·D48).

### `image` — 위키미디어에 걸어 두는 그림 (D29)

```yaml
image:
  file: Nike of Samothrake Louvre Ma2369 n4.jpg   # "File:" 은 빼고 파일 이름만
  caption: 사모트라케의 니케, 루브르
  license: Public domain
```

파일이 실제로 있는지는 `npm run images` 가 따로 확인한다. 감사 코드는 W09 다.

### `figures` — 곁들이는 그림 (D49)

대표 그림 한 장으로 모자랄 때 뒤에 이어 붙인다. `kicker` 는 이 장이 무엇인지 알리는 짧은 딱지이며,
지금 쓰는 것은 **조각으로는 · 그림으로는 · 유물로는 · 그 자리는 · 그 이름은 · 옛 지도로는 · 옛 성도로는** 일곱 갈래다.
"그 이름은" 은 그 이름이 지금 남아 있는 자리를 보여 줄 때 쓴다. 물총새(할키온), 공작(아르고스), 샹젤리제(엘리시온)가 그렇다.

```yaml
figures:
  - file: 'Nike of Samothrake Louvre Ma2369 n4.jpg'
    kicker: 조각으로는
    caption: '사모트라케의 니케, 기원전 2세기. …'
    license: Public domain
```

같은 파일을 한 카드에 두 번 걸거나 `image` 와 겹치면 감사 W12 가 잡는다.

**카드에 그림이 걸리는 순서**는 이렇다.

1. 이 카드 자신의 그림 (`image`)
2. 흔적이면, 그 이름이 나온 원천의 그림 — 빌려 오는 것이 아니라 **함께** 건다
3. 별자리 도형과 그 옛 성도 (`constellation`)
4. 실제 지도 (`map`)
5. `figures` 에 적은 것들

### `constellation` — 맨눈으로 보이는 별자리 모양 (D46)

```yaml
constellation: orion        # app/components/Constellation.tsx 의 도형 이름
```

### `map` — 실제 지도 위의 자리 (D48)

위경도를 적으면 화면이 위키미디어 공용의 실제 지도를 걸고 그 위에 자리를 찍는다.
그리고 그 좌표를 그대로 넘기는 **구글 지도 링크**를 함께 단다. 열쇠도 지도 서버도 필요 없다.

```yaml
map:
  lat: 41.118               # 필수. -90 ~ 90
  lng: 29.068               # 필수. -180 ~ 180
  span: 17                  # 선택. 보여 줄 경도 폭(도). 없으면 바탕 지도 전체
  base: med                 # 선택. med(지중해) | world(세계). 없으면 med
  spread: 2.6               # 선택. 한 점이 아니라 넓은 자리면 그 반지름(도). 점 대신 옅은 원을 그린다
  label: 보스포루스 해협     # 선택. 없으면 카드 이름을 쓴다
  note: 흑해에서 내려오는 물길이 여기서 가장 좁아진다.   # 선택. 지도 아래 한두 문장
  open: false               # 선택. 구글 지도 링크를 감출 때만 적는다
```

바탕 지도 목록과 각각이 덮는 범위는 `app/lib/map.ts` 의 `MAP_BASES` 에 있다.
그 범위를 벗어난 좌표, 뒤집힌 위경도, 없는 바탕 지도 이름은 감사 W11 이 잡는다.
대서양처럼 지중해 지도가 덮지 못하는 자리는 `base: world` 를 적어야 한다.
