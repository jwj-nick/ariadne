...
| 마지막 세션 | 2026-09-11 (세션 "adriane", R11 — 배움과 복습 분리·퀴즈 그림·묶음 퀴즈·실) |
...
| A10 | 격자·섬네일의 잘림 다듬기 | 반나절 | `object-cover` 라서 세로로 긴 조각상은 머리가 잘린다. 트로이 목마 도기는 목마가 안 보인다 |
| A11 | 아이 눈높이 실사용 점검 | 반나절 | 3지선다는 있지만 아이가 **계속 열 이유**가 있는지는 확인된 적이 없다 (U7) |
| A12 | 실을 더 쓰기 | 그때그때 | 지금 6편. 자동차 이름·조각상·영화 제목 쪽에 아직 무리가 남아 있다 (D39) |
